...
#idx=[x for x in range(len(train_data))]
#idx_loader=td.DataLoader(dataset=idx,batch_size=batch_size)
#train_loaders=[]
#print(idx_loader)
#print('..split the merge list')
#for index in idx_loader:
    #train_index=get_split_list(index.tolist())
#    train_loader=get_train_loader(index.tolist())
...
b_size=int(len(train_data)/1000)
b_size+=1
for step in range(b_size):
    if step == b_size-1:
        train=torch.FloatTensor(train_data[1000*step:])
        label=torch.FloatTensor(train_label[1000*step:])
    else:
        train=torch.FloatTensor(train_data[1000*step:1000*(step+1)])
        label=torch.FloatTensor(train_label[1000*step:1000*(step+1)])
    ...
    optimizer.step()
    #if step%100 ==0:
    print ('step-%d, data-size [%d] Loss: %.4f' 
            %(step+1, len(train),loss.data[0]))
